...
const PATH = path.join(argv.a || argv.path ? process.cwd() : `${__dirname}/..`, ...((argv.a || argv.path || './ui').replace(/^\.\/?/g, '').replace(/\/?$/g, '').split('/')))
...
const buildIndex = (dir, extensions) => new Promise((resolve, reject) => {
  buildUiKitAST(dir, extensions)
    .then(({categories}) => readFileAsync(path.join(__dirname, INDEX_TEMPLATE))
      .then((data) => data.toString('utf8').replace(TEMPLATE_REGEXP, JSON.stringify(categories)))
    )
    .then((data) => writeFileAsync(path.join(__dirname, '..', 'pages', 'index.js'), data))
    .then(() => resolve(true))
...
const runNextJS = () => {
  nextjs = spawn(NEXTJS_PATH, {cwd: path.join(__dirname, '..')}, [`-p ${PORT}`])
